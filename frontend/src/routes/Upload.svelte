<script>
  ...
  onMount(async () => {
      ...
      ijazahList = myLegalize.getCertificateList();
    } catch(e) {
      errorServiceHandling(e)
      if (Cookies.get('token') == null) {
        location = PATH_URL.LOGIN  
              ...
              {:else if ijazah.getLegalize().getStatus() === 2}
                ...
                {#if !ijazah.getLegalize().getRating() && !hideRate}
                  ...
                  <div id="formRating">
                    ...
                    <button on:click="{sendRating}" type="button" class="px-6 py-2 ml-4 text-lg text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 hover:bg-indigo-500">
                      Kirim penilaian
                    </button>
                  </div>
                {/if}
